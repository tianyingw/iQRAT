  ...
  if(length(ind_common) > 0 & length(ind_rare) > 0){
    ...
    if(!is.null(SingleScore)){
      ...
      out = list(pval = pval_all,  p_Wilcoxon =  pcauchy(mean(tan((0.5-c(pval_S_c[1],pval_S_r[1]))*pi)), lower.tail = FALSE),
                 p_Normal =  pcauchy(mean(tan((0.5-c(pval_S_c[3],pval_S_r[3]))*pi)),lower.tail = FALSE),
    ...
    if(!is.null(SingleScore)){
      ...
      out = list(pval = pval_S_CT3_r, p_Wilcoxon = pval_S_r[1], p_Normal = pval_S_r[3], case = "RareOnly", method.type = method.type )
